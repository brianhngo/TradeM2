...
export default function Map({
  ...
  const handleLocateProduct = async (product) => {
    const location = product.productDetails.location;

    // Call locateProduct first
    await locateProduct(location);

    // Call updateMapLocation with the location
    const [lat, lon] = location.split(',').map(parseFloat);
    const coordinates = [lat, lon];

    // Use the passed updateMapLocation function to update the map
    ...
    <div className="mapContainer">
      <MapContainer
        ...
        {highlightedProductLocation && (
          <Marker
            position={highlightedProductLocation}
            icon={customIcon}></Marker>
        )}

        <MarkerClusterGroup chunkedLoading>
          {filteredProducts.map((marker, idx) => (
